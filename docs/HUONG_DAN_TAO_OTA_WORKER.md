...
Tại Tab **Settings** > **Variables and Secrets**, hãy khai báo các biến sau:
...
Từ phiên bản v8.2, người dùng không cần sửa KV thủ công nữa. Hãy sử dụng giao diện Web:

1. **Truy cập**: `https://ten-worker-cua-ban.workers.dev/admin`
2. **Đăng nhập**: Nhập `AdminPass` đã cài ở Bước 4.
3. **Phê duyệt**: Các máy mới cài đặt sẽ hiện ở bảng **CONNECTED DEVICES**. Bạn chỉ cần bấm **APPROVE** là xong.
4. **Cấu hình Version**: Bạn có thể đổi bản `Software Version` và `Binary URL` ngay trên Web. Các máy đang bật **Auto-Update** sẽ tự động thấy và nâng cấp sau mỗi 10 phút.

---

*(Mọi thiết bị điện thoại giờ đây đều có thể Discovery tự động, Quản trị viên chỉ việc theo dõi và Duyệt!)*
